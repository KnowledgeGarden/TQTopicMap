...
var TopicModel = module.exports = function(environment) {
  ...
  self.__hasRelation = function(topic, relationType, targetlocator) {
	...
	for (var i=0;i<len;i++) {
		r = relns[i];
		if (r.locator === targetlocator) {return true;}
	}
	return false;
  },
  ...
  self.nodeFromJSON = function(json, callback) {
	  ...
	  if (json.subof) {
		  self.newSubclassNode(json.locator,json.subOf,json.label,json.details,constants.ENGLISH,
				  constants.SYSTEM_USER,json.smallIcon,json.largeIcon,false,defaultCredentials, function(err,data){
			  dataProvider.putNode(data,function(err,data) {
				  callback(err);
			  });
		  });
	  } else {
		  self.newNode(json.locator,json.label,json.details,constants.ENGLISH,
				  constants.SYSTEM_USER,json.smallIcon,json.largeIcon,false, function(data){
			  dataProvider.putNode(data,function(err,data) {
				  callback(err);
			  });
		  });

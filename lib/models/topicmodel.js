...
//      tuple.setSignature(signature);
      ...
      if (!isPrivate) {
        ...
        sourceNode.addRestrictedRelation(relationTypeLocator,relationTypeLocator,targetNode.getSmallImage(),
				targetNode.getLocator(),label, targetNode.getNodeType(),"t");
      }
   ...
   //   }
      if (!isPrivate) {
        ...
        targetNode.addRestrictedRelation(relationTypeLocator,relationTypeLocator,sourceNode.getSmallImage(),
				sourceNode.getLocator(),label, sourceNode.getNodeType(),"s");
      }
         ...
         dataProvider.putNode(sourceNode, function(err,data) {
          if (err) {errors += err;}
           dataProvider.putNode(targetNode, function(err,data) {
            if (err) {errors += err;}
            if (isTransclude) {
            	//TODO must add Transclude pivot between user and target node
            	//TODO create UserDocumentTransclude relation
            }
            callback(errors,tuple);
          });
        });
      });
    });
...
//    tuple.setSignature(signature);
    ...
    if (!isPrivate) {
        sourceNode.addPivot(relationTypeLocator,relationTypeLocator,targetNode.getSmallImage(),
				targetNode.getLocator(),label, targetNode.getNodeType(),"t");
    } else {
        sourceNode.addRestrictedPivot(relationTypeLocator,relationTypeLocator,targetNode.getSmallImage(),
				targetNode.getLocator(),label, targetNode.getNodeType(),"t");
    }
    label = sourceNode.getLabel(language);
    if (!label) {
    ...
    if (!isPrivate) {
        targetNode.addPivot(relationTypeLocator,relationTypeLocator,sourceNode.getSmallImage(),
				sourceNode.getLocator(),label, sourceNode.getNodeType(),"s");
    } else {
        targetNode.addRestrictedPivot(relationTypeLocator,relationTypeLocator,sourceNode.getSmallImage(),
				sourceNode.getLocator(),label, sourceNode.getNodeType(),"s");
    }

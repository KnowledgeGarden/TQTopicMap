...
SubjectProxy.prototype.addInfoBox = function(jsonString) {
	var l = this.properties.infL;
	if (!l) {l = [];}
	var where = l.indexOf(jsonString);
	if (where < 0) {l.push(jsonString)};
	this.properties.infL = l;
};

SubjectProxy.prototype.removeInfoBox = function(jsonString) {
	var l = this.properties.infL;
	if (l) {
		var where = l.indexOf(jsonString);
		if (where > -1) {
...
SubjectProxy.prototype.updateInfoBox = function(oldJsonString, newJsonString) {
	var l = this.properties.infL;
	if (l) {
		//remove old one
		var where = l.indexOf(oldJsonString);
		if (where > -1) {
			l = l.splice(where,1);
		}
		//push new one
		l.push(newJsonString);
		this.properties.infL = l;
	}
};

SubjectProxy.prototype.listInfoBoxes = function() {
...
SubjectProxy.prototype.addRestrictedRelation = function(relationType, relationLabel, //documentType,
		documentSmallIcon,targetLocator, targetLabel) {
...
* @param subject
*/
SubjectProxy.prototype.addChildNode = function(contextLocator, smallIcon, locator,subject) {

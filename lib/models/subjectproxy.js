...
SubjectProxy.prototype.setObject = function(object) {
  this.properties.tupOb = object;
};

SubjectProxy.prototype.getObject = function() {
  return this.properties.tupOb;
};
...
SubjectProxy.prototype.addPivot = function(relationType,pivotLocator, relationLabel,
		documentSmallIcon,targetLocator, targetLabel, nodeType, sORt) {
	console.log('SubjectProxy.addPivot '+documentSmallIcon+' '+pivotLocator+' '+relationLabel+
	...
	for (var i=0;i<len;i++) {
		tx = l[i];
        if (tx.relationLocator === pivotLocator) {
            notFound = false;
            break;
        }
	}
...
SubjectProxy.prototype.addRestrictedPivot = function(relationType, pivotLocator, relationLabel,
		documentSmallIcon,targetLocator, targetLabel, nodeType, sORt) {
...
*/
SubjectProxy.prototype.addRelation = function(relationType,relationLocator, relationLabel,
		documentSmallIcon,targetLocator, targetLabel, nodeType, sORt) {
	console.log('SubjectProxy.addRelation '+documentSmallIcon+' '+relationLocator+' '+relationLabel+
				targetLocator+' '+targetLabel);
	var rx = struct(relationType,relationLocator,relationLabel,
			documentSmallIcon, targetLocator, targetLabel, nodeType, sORt);
	...
	for (var i=0;i<len;i++) {
        ...
        if (tx.relationLocator === relationLocator) {
            notFound = false;
            break;
        }
	}
...
SubjectProxy.prototype.addRestrictedRelation = function(relationType, relationLocator,relationLabel,
		documentSmallIcon,targetLocator, targetLabel, nodeType, sORt) {
...
SubjectProxy.prototype.setIsLive = function(isLive) {
  var ip = 'false';
  if (isLive) {
    ip = 'true';
  }
  this.properties.isLiv = ip;
};

SubjectProxy.prototype.getIsLive = function() {
  var ip = this.properties.isLiv;
  if (!ip) {
    return true; // not set = default true
  }
  if (ip === 'true') {
    return true;
  } else {
...
SubjectProxy.prototype.getProperties = function() {
...
SubjectProxy.prototype.addSetValue = function(key, value) {
    var lst = this.properties[key];
    if (!lst) { lst=[]; };
    var where = lst.indexOf(value);
    if (where < 0) {
        lst.push(value);
    }
    this.properties[key] = lst;
};

SubjectProxy.prototype.removeCollectionValue = function(key, value) {
    var lst = this.properties[key];
    if (lst) {
 		var where = lst.indexOf(value);
		if (where > -1) {

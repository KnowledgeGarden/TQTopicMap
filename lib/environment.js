...
var TopicMapEnvironment = module.exports = function(callback) {
	...
	fs.readFile(__dirname+"/../config/config.json", function(err, configfile) {
		configProperties = JSON.parse(configfile);
		lgr.configure(__dirname+"/../config/logger.json");
		...
		fs.readFile(__dirname+"/../config/mappings.json", function(err,mapfil) {

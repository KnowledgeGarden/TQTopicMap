...
DataProvider.prototype.getNodeByLocator = function (locator,credentials,callback) {
	...
	if (!locator) {
		var empty;
		callback("DataProvider.getNodeByLocator missing locator", empty);
	}
	var self = this;
	var _jopts = {};
	_jopts._index = constants.TOPIC_INDEX;
    ...
    var dx = this.cache.get(locator);
    if (dx) {
        _checkCredentials(dx.toJSON(), credentials, function(err) {
            console.log("DP-1 "+err);
            if (!err) {
                 callback(err,dx);
            } else {
                self.env.logError("DP.getNodeByLocator "+locator+" "+err);
                 callback(err,dx);
            }
       });
    } else {
        this.ESClient.getDocument(_jopts, locator, function(err,data) {
           console.log('DataProvider.getNodeByLocator '+locator+" "+data);

            var error = '';
            if (err) {
                error+=err;
                self.env.logError("DataProvider.getNodeByLocator-1 "+locator+" "+error);
            }
            if (data) {
                var jp = data._source;
                self.env.logDebug("DataProvider.getNodeByLocator-1 "+JSON.stringify(jp));
                _checkCredentials(jp, credentials, function(err) {
                    ...
                    self.env.logDebug("DataProvider.getNodeByLocator+ "+error+" | "+locator+" | "+dx.toJSON());
                    sclf.cache.set(locator,dx);

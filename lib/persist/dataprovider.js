 ...
 */
DataProvider.prototype.getNodeByLocator = function getNode(locator,credentials,callback) {
	console.log('DataProvider.getNodeByLocator '+locator);
	var opts = _PREFIX+"}";
...
DataProvider.prototype.listNodesByLabel = function(label, language, start, count, credentials, callback) {
	...
	__listNodesByQuery(this.ESClient, query,function(err,dx) {
		callback(err,dx);
	});
};
...
var __listNodesByQuery = function(client, query, callback) {
  client.search(/*jopts,*/query,function(err,data) {
    var hits;
    var result;
    if (data) {
      hits = data.hits.hits;
      var count = hits.length;
      console.log("DataProvider.__listNodesByQuery "+JSON.stringify(query)+" "+count);
      if (count > 0) {

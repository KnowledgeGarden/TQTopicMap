...
function _checkCredentials(self, json, credentials, callback) {
	...
	//see if it's private
    self.env.logDebug("DataProvider._checkCredentials "+json+" "+json.lox+" "+pvt+" "+isLive);
	if (isLive && pvt === 'true') {
	...
	self.getNodeByLocator(locator, credentials, function(err, node) {
		...
		if (node) {
			...
			var image = node.getSmallImage(),
				typ = node.getNodeType(),
				subs,
				instances;
			theTreeNode = new treenode(lox,label, image, typ);
 ...
 */
var __getNodeByQuery = function(self, ESClient, query, credentials, callback) {
		...
		if (data) {
			...
			if (count > 0) {
				...
				if (jp) {
					...
					//check credentials
					_checkCredentials(myself, jp, credentials, function(err) {
						if (!err) {
							dx = new SubjectProxy(jp);
						} else {
							ex = err;
...
DataProvider.prototype.getNodeByURL = function(url, credentials, callback) {
	var query = this.queryDSL.findNodeByURL(url),
		self = this;
	__getNodeByQuery(self, this.ESClient, query,credentials, function(err,data) {

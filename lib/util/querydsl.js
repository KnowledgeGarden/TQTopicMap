...
var QueryDSL = module.exports = function() {
  ...
  that.listNodesByKey = function(key,value,start,count) {
    return matchQuery(key, value,start,count);
  };
   ...
   * @param count
   */
  //TODO generalize this to an array of text fields
  that.listByText = function(textFieldA,textFieldB, textFieldC, textFieldD, queryString, start, count) {
    ...
    if (textFieldC !== null) {l.push(textFieldC);}
    if (textFieldD !== null) {l.push(textFieldD);}
    m.fields = l;
    if (count > -1) {
    	query.size = count;
    	query.from = start;
 ...
 /**   query.sort = "_score";
   ...
   */
  //{"sort":{"srtDt":{"order":"desc"}},"query":{"field":{"inOf":"BlogNodeType"}}} OLD
  //{"query":{"match":{"instanceOf":"SortTestType"},"sort":{"srtDt":{"order":"desc"}}}} FAILS
  //{"query":{"term":{"instanceOf":"SortTestType"}},"sort":{"srtDt":{"order":"desc"}}} NO HITS
  
  that.sortedDateTermQuery = function(property,value, start, count) {
    var query = {};
    if (count > -1) {
    	query.size = count;
    	query.from = start;
    ...
    var prop = {};
    prop[property] = value;
    //technically, match should be "term"
    q.match=prop;
   query.query = q;
   query.sort = s;
    return query;
   ...
   * @return query for sorting hits 
   */
  //{"sort":{"tpC":{"order":"desc"}},"query":{"field":{"inOf":"TagNodeType"}}}fails
  that.sortedTupleCountTermQuery = function(property,value, start, count) {
...
/*
  ...
  function matchQuery(property,value, start, count) {
		var query = {};
	    if (count > -1) {
	    	query.size = count;
	    	query.from = start;
